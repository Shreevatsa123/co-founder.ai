...
export const PromptManager: React.FC<PromptManagerProps> = ({ blueprint, onUpdateBlueprint, onClose, isInline = false }) => {
  ...
  const content = (
    <>
      {/* Header - Only show if inline or if modal header is desired inside */}
      {!isInline && (
      ...
      {/* If inline, we might want a different header or none */}
      {isInline && (
         <div className="max-w-4xl mx-auto mb-8 text-center">
            <h2 className="text-3xl font-bold text-slate-900 mb-2 font-display">Build Plan</h2>
            <p className="text-slate-500">Master prompts for your AI coding assistant.</p>
         </div>
      )}

      {/* Content */}
      <div className={`flex-1 overflow-y-auto ${isInline ? 'w-full' : 'p-6 bg-slate-50/50'}`}>
        {!blueprint.promptPlan || blueprint.promptPlan.length === 0 ? (
          ...
          <div className={`space-y-8 max-w-3xl mx-auto ${isInline ? 'pb-20' : ''}`}>
             <div className="flex justify-between items-center mb-4">
                <span className="text-xs font-bold uppercase text-slate-400">Sequence</span>
                <button 
                  onClick={handleGenerate}
  ...
  if (isInline) {
    return (
      <div className="w-full h-full bg-transparent overflow-y-auto p-8 pr-24 animate-in fade-in slide-in-from-right-4 duration-300">
         {content}
      </div>
    );
  }
